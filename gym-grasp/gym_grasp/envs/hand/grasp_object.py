...
class ManipulateEnv(hand_env.HandEnv, utils.EzPickle):
    ...
    def _reset_sim(self):
        ...
        joint_names = ["robot0:zslider",  # Lite用
                       "robot0:rollhinge",
    ...
    def _get_obs(self):
        ...
        # object_qvel = self.sim.data.get_joint_qvel("scissors_hinge_2:joint")  # エラーが出るし, どう使えるのか不明なのでコメントアウト
        achieved_goal = self._get_achieved_goal().ravel()  # はさみの角度(1次元)の取得
        sensordata = self._get_contact_forces()

        observation = np.concatenate([robot_qpos, robot_qvel, achieved_goal, sensordata])
...
class GraspObjectEnv(ManipulateEnv):
    def __init__(self, target_position='random', target_rotation='fixed', reward_type="not_sparse"):
        super(GraspObjectEnv, self).__init__(
            ...
            target_angle_range=np.array([[0.0, 1.0]]),  # はさみの可動範囲である0度〜60度
