...
class OperateExecution():
    ...
    def plot_angle_history(self, desired_goal):
        ...
        plt.plot(self.time_history, self.angle_history, label="Scissors Angle [rad]")
        ...
        # サブディレクトリ名を生成（例：desiredgoal_0.4/Kp0.3_Ki0.0_Kd0.0）
        desiredgoal_dir_name = f"desiredgoal_{desired_goal:.1f}"
        pid_dir_name = f"Kp{self.Kp}_Ki{self.Ki}_Kd{self.Kd}"
        sub_dir_path = os.path.join(self.save_dir, desiredgoal_dir_name, pid_dir_name)
        ...
        # ファイル名にPIDパラメータとdesired_goalを組み込む
        now_jst = datetime.utcnow() + timedelta(hours=9)  # UTCに9時間加算してJSTを取得
        timestamp = now_jst.strftime("%Y%m%d%H%M%S")  # 'YYYYMMDDHHMM'形式
    ...
    def generate_performance_report(self, desired_goal, settling_time, overshoot, peak_time, steady_state_error):
        ...
        timestamp_filename = now_jst.strftime("%Y%m%d%H%M%S")  # 'YYYYMMDDHHMM'形式
    ...
    def run_posture(self, joint_goal):
        ...
        while not self.stop_requested:
            if hasattr(self, 'current_angle'):
                ...
                self.pc1_score = np.clip(self.pc1_score, -0.45, 0.5)
                ...
                rospy.loginfo(f"Moving!!!!!(Base on angle: {current_ag:.4f})")
                # rospy.loginfo(f"(Adjusted_targets: {adjusted_targets})")

                # ここでjoint_trajectory_controllerへ送信
                ...
                # self.client.wait_for_result(timeout=rospy.Duration(2.0))
