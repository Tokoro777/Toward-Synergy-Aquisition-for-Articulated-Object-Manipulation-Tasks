...
def plot_scatter(pc1_values, ag_values):
    ...
    ax.set_xlabel('PC2 Values')
    ax.set_ylabel('Achieved Goal Values (ag)')
    ax.set_title('PC2 vs Achieved Goal Values')
    ax.legend()
...
euler_values = postures[:, -1]  # euler_angleの値を取得

# 相関係数の計算
correlation_coefficient = np.corrcoef(pc2_values, euler_values)[0, 1]
print(f"Correlation Coefficient between PC2 and ag: {correlation_coefficient:.4f}")

# 別スレッドでプロットを表示
plot_thread = threading.Thread(target=plot_scatter, args=(pc2_values, euler_values))
plot_thread.start()
...
while pos_num < len(postures):
    ...
    euler = euler_values[pos_num]

    if t == 1:  # 表示するポスチャの番号とPC2値とeuler値を表示, postureを500step表示
        print(f"Posture {pos_num}: PC1 = {pc2:.4f}, ag = {euler:.4f}")
