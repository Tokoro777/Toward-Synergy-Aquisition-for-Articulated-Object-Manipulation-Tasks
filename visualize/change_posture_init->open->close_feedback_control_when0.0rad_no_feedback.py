...
# シミュレーションループ
t = 0
desired_goal = 0.3  # 初期目標角度は0.4rad
...
while True:
    ...
    if desired_goal == 0.3:
        ...
        sim.data.ctrl[:] = actuation_center[:] + (inverse_posture_2[0]) * actuation_range[:]  #  + pid_outputを削除. 0.0rad時はフィードバック制御なし
    ...
    if t == 1000:
        ...
        if loop_count == 2:
            ...
            plt.axhline(y=0.3, color='r', linestyle='--', label="Target Scissors Angle (0.4rad)")
            plt.xlabel('Steps')
        ...
        desired_goal = 0.3  # 初期目標角度を0.4radにリセット
        t = 0  # タイマーリセット
