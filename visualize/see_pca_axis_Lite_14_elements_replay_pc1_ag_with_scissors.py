...
ag_values = postures[:, -1]  # agの値を取得, posturesに保存されているagは現実と値が異なる。。。。
...
while pos_num < len(postures):
    ...
    if t > 500:  # 初期化部分
        print(_get_achieved_goal())  # はさみの角度出力
        t = 0
        pos_num += 1
    ...
    sim.data.ctrl[:-1] = actuation_center[:-1] + postures[pos_num][:-1] * actuation_range[:-1]  # アクチュエータが14個で、データセットからagを除くパターン
